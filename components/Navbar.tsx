...
function NavLink({ href, label }: { href: string; label: string }) {
  ...
  return (
    <a href={href} onClick={handleClick} onMouseEnter={scramble} onMouseLeave={reset}
      className="text-[0.72rem] tracking-[0.04em] px-3.5 py-1.5 rounded-full transition-all no-underline"
      style={{ color: "var(--text-dim)" }}
      onMouseOver={e => { (e.currentTarget as HTMLAnchorElement).style.background = "var(--card)"; (e.currentTarget as HTMLAnchorElement).style.color = "#FF6B5B"; }}
      onMouseOut={e => { (e.currentTarget as HTMLAnchorElement).style.background = "transparent"; (e.currentTarget as HTMLAnchorElement).style.color = "var(--text-dim)"; }}
    >{text}</a>
  );
}
...
<nav
    ...
    >
      <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
        ...
        className="flex items-center gap-2.5 mr-7 hover:opacity-75 transition-opacity"
      >
        <div className="w-[30px] h-[30px] rounded-full flex items-center justify-center text-white font-bold text-[0.7rem] flex-shrink-0"
      ...
      <div className="hidden sm:block">
        ...
        <a href="mailto:thisispurab01@gmail.com"
          ...
          onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = "#FF6B5B"; (e.currentTarget as HTMLAnchorElement).style.color = "#fff"; }}
          onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = "var(--text)"; (e.currentTarget as HTMLAnchorElement).style.color = "var(--surface)"; }}
        >
          Say Hello
        </a>
      </div>
